...
(function () {
  ...
  const textedNoteText = () => { const n = realNotes().find((x) => TEXTED_NOTE.test(x.textContent || '')); return n ? (n.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 140) : ''; };
  ...
  function publishShared() {
    try {
      localStorage.setItem('sa_shared_queue', JSON.stringify({ ts: Date.now(), q: callQueue.map((l) => ({
        ...
        issue: (typeof l.issue === 'string' ? l.issue : (l.issue === null ? null : undefined)), size: l.size || '', act: l.act || undefined, texted: l.texted || undefined, tnote: l.tnote || undefined,
      })) }));
    } catch (e) {}
  }
  function saveState() {
